...
struct node_t *node_create(int n_operators, int n_terminals, int max_depth,
                           _Bool full) {
...
double calculate(struct node_t *node, double *sample) {
  if (node->left == NULL) {
    ...
    case 3:; // /
      ...
      if (r == 0)
        r = 1; // protected division
      return calculate(node->left, sample) / r;
    default:
      printf("ERRO: Node.calculate\n");
      abort();
    }
  }
}

struct node_t *getRandomNode_aux(struct node_t *node, int value) {
  if (value == 0) {
    return node;
  }
  ...
  if (value <= left_size) {
    return getRandomNode_aux(node->left, value - 1);
  } else {
    return getRandomNode_aux(node->right, value - left_size - 1);
  }
}

struct node_t *getRandomNode(struct node_t *node) {
  int value = rand() % getSize_node(node);
  return getRandomNode_aux(node, value);
}
...
char *toString_node(struct node_t *node, char **ops, char **term) {
  if (node->left != NULL) {
    ...
    char *ret = calloc(6 + strlen(ops[node->value]) + strlen(l) + strlen(r) + 1,
                       sizeof(char));
    strcat(ret, "( ");
    strcat(ret, l);
    strcat(ret, " ");
