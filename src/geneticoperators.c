...
struct individual_t **getElite(int p_size, struct individual_t **population,
                               int e_size) {
...
struct individual_t *tournament(int population_size,
                                struct individual_t **population, int t_size) {
  int picked = rand() % population_size;
  int i,p;
  for (i = 1; i < t_size; ++i) {
    p = rand() % population_size;
    if (p < picked) {
      picked = p;
    }
  }
...
struct individual_t **getOffspring(int p_size, struct individual_t **population,
                                   int t_size) {
  int isCross = rand() % 2;
...
struct individual_t **STXO(int p_size, struct individual_t **population,
                           int t_size) {
  ...
  struct node_t *r1 = getRandomNode(h1);
  struct node_t *r2 = getRandomNode(h2);
...
struct individual_t **STMUT(int p_size, struct individual_t **population,
                            int t_size) {
  struct individual_t *i1 = tournament(p_size, population, t_size);
...
struct individual_t **discardDeep(struct individual_t **population,
                                  int max_depth) {
  int MAX_CHILDREN = 2;
  struct individual_t **ret =
      calloc(MAX_CHILDREN + 1, sizeof(struct individual_t *));
  int track = 0;
