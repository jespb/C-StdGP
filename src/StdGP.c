...
struct StdGP_t* stdgp_create(int n_operators, char** operators, int n_terminals,
	...
	stdgp->population = malloc( sizeof(struct individual_t*) * (population_size +1) );
	for (int i = 0; i < population_size; ++i){
		stdgp->population[i] = individual_create(n_operators, operators, n_terminals, terminals, max_depth);
	}
...
_Bool stoppingCriteria(struct StdGP_t* stdgp){
	...
	_Bool perfectTraining = getFitness(stdgp->bestIndividual, stdgp->n_training_samples, stdgp->training_X, stdgp->training_Y) == 1;
...
void fit(struct StdGP_t* stdgp){
	...
	while (stdgp->currentGeneration < stdgp->max_generation){
		...
		if (!stoppingCriteria(stdgp)) {
			...
			if (stdgp->verbose && stdgp->currentGeneration%1==0) printf("  > Execution Time: %ldms\n", duration );
    	}
		...
		stdgp->sizeOverTime[ stdgp->currentGeneration ] = getSize(stdgp->bestIndividual);
		stdgp->timeOverTime[ stdgp->currentGeneration ] = duration/1000.0;

		//printf("%f\n", getAccuracy(stdgp->bestIndividual, stdgp->n_training_samples, stdgp->training_X, stdgp->training_Y));
...
void nextGeneration(struct StdGP_t* stdgp){
	...
	for (int i = 0; i < stdgp->population_size; ++i){
		getFitness(stdgp->population[i], stdgp->n_training_samples, stdgp->training_X, stdgp->training_Y );
		//printf("%ld\n", stdgp->population[i]);
	}


	// Sorts the population
	qsort( stdgp->population, stdgp->population_size, sizeof(struct individual_t*), compare);
	...
	struct individual_t** new_population = calloc( stdgp->population_size +1, sizeof(struct individual_t*));
	struct individual_t** elite = getElite(stdgp->population_size, stdgp->population, stdgp->elitism_size);
	...
	for (int i = stdgp->elitism_size; i < stdgp->population_size; ){
		struct individual_t** offspring = getOffspring(stdgp->population_size, stdgp->population, stdgp->tournament_size);
		struct individual_t** afterDiscard = discardDeep(offspring, stdgp->limit_depth);
		free(offspring);
	...
	// Generation Log
	if (stdgp->verbose && stdgp->currentGeneration%1==0) printf("  > Training: %.4f  // Test: %.4f      ", 
		getAccuracy(stdgp->bestIndividual, stdgp->n_training_samples, stdgp->training_X, stdgp->training_Y), 
		getAccuracy(stdgp->bestIndividual, stdgp->n_test_samples, stdgp->test_X, stdgp->test_Y) 
		);
...
int* getSizeOverTime(struct StdGP_t* stdgp){
	return stdgp->sizeOverTime;
}

double* getTimeOverTime(struct StdGP_t* stdgp){
	return stdgp->timeOverTime;
}
